...
public class WorldSpawner : MonoBehaviour
{
    ...
    void SpawnOilInChunk(GameObject chunk)
    {
        ...
        int oilCount = Random.Range(0, 3); // 0–2 oil spills per chunk

        for (int i = 0; i < oilCount; i++)
        {
            Vector3 spawnPos = validPositions[Random.Range(0, validPositions.Count)];

            Instantiate(oilPrefab, spawnPos, Quaternion.identity, chunk.transform);
        }
    }
}
